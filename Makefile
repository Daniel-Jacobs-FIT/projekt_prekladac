...
TOPDOWN-PARSER-LIB-OBJS=${SRCDIR}/topdown-parser.o
TOPDOWN-PARSER-TEST-OBJS=${SRCDIR}/topdown-parser-tests.o

PSA-STACK-LIB-OBJS=${SRCDIR}/psa_stack.o
PSA-STACK-TEST-OBJS=${SRCDIR}/psa_stack-test.o

ALL-OBJS=${SCANNER-TEST-OBJS} ${SCANNER-LIB-OBJS} ${SYMTAB-TEST-OBJS} ${SYMTAB-LIB-OBJS} ${BOTUP-PARSER-LIB-OBJS} ${BOTUP-PARSER-TEST-OBJS} ${PSA-STACK-LIB-OBJS} ${PSA-STACK-TEST-OBJS} ${TOPDOWN-PARSER-LIB-OBJS} ${TOPDOWN-PARSER-TEST-OBJS} ${UTILS-OBJS}
...
TOPDOWN-PARSER-TEST-NAME=topdown-parser-tests

${TOPDOWN-PARSER-TEST-NAME}: ${TOPDOWN-PARSER-LIB-OBJS} ${TOPDOWN-PARSER-TEST-OBJS} ${BOTUP-PARSER-LIB-OBJS} ${SCANNER-LIB-OBJS} ${SYMTAB-LIB-OBJS} ${PSA-STACK-LIB-OBJS} ${UTILS-OBJS}
	${CC} ${CFLAGS} $^ -o $@

run-${TOPDOWN-PARSER-TEST-NAME}: ${TOPDOWN-PARSER-TEST-NAME}
	src/test-skript.sh top-down-parser
...
run-${BOTUP-PARSER-TEST-NAME}: $(BOTUP-PARSER-TEST-NAME)
	src/test-skript.sh bottom-up-parser

clean:
	rm -f ${ALL-OBJS} ${SCANNER-TEST-NAME} ${SYMTABLE-TEST-NAME} ${BOTUP-PARSER-TEST-NAME} ${PSA-STACK-TEST-NAME} ${TOPDOWN-PARSER-TEST-NAME}
